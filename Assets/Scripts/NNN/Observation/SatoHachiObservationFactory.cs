        ...
        private static List<NNNActionDefinition> Actions()
        {
            return new List<NNNActionDefinition>
            {
                new NNNActionDefinition(InvestigateIndoor, "室内での過ごし方を調べる", "休息と活動、玄関へ向かうタイミングを調べる。", NNNActionKind.Investigation,
                    intentText: "室内の活動・探索不足が外出要求に関係しているか",
                ...
                new NNNActionDefinition(InvestigatePast, "以前の生活圏を調べる", "派遣前の足取りと、繰り返し立ち寄った場所を確認する。", NNNActionKind.Investigation,
                    intentText: "外出要求が以前の生活場所と関係しているか",
                ...
                new NNNActionDefinition(InvestigateRoute, "佐藤宅から商店街までの環境", "道幅、交通量、現地での移動経路を調べる。", NNNActionKind.Investigation,
                    intentText: "商店街までの移動に、どんな条件があるか",
                ...
                new NNNActionDefinition(InstallTower, "タワーと室内巡回路を整える", "登れる場所と窓辺までの高低差を用意する。", NNNActionKind.Operation,
                    intentText: "室内の探索場所を増やすと、行動が変わるか",
                    requiredKnowledgeTags: new[] { WantsOutside, IndoorOpportunity }, discoveryKnowledgeTags: new[] { IndoorOpportunity },
                ...
                new NNNActionDefinition(ArrangePlay, "室内で遊ぶ準備をする", "短い遊びのためにおもちゃを取り出しやすくする。", NNNActionKind.Operation,
                    intentText: "遊ぶ機会を増やすと、その後の行動が変わるか",
                    requiredKnowledgeTags: new[] { IndoorOpportunity }, discoveryKnowledgeTags: new[] { IndoorOpportunity }, effect: new OperationEffect(new[] { PlayPrepared })),
                new NNNActionDefinition(PrepareWindow, "安全な窓辺の居場所を整える", "閉じた窓のそばに、座って眺める場所を作る。", NNNActionKind.Operation,
                    intentText: "窓辺に居場所を用意すると、過ごし方が変わるか",
                    requiredKnowledgeTags: new[] { IndoorOpportunity }, discoveryKnowledgeTags: new[] { IndoorOpportunity }, effect: new OperationEffect(new[] { WindowPrepared })),
                new NNNActionDefinition(PrepareGear, "安全な外出の道具を揃える", "サイズの合うハーネスと、道路区間で使うキャリーを準備する。", NNNActionKind.Operation,
                    intentText: "外出道具を用意すると、一緒の外出を検討するか",
                    requiredKnowledgeTags: new[] { WantsOutside, FamiliarStreets }, discoveryKnowledgeTags: new[] { FamiliarStreets }, effect: new OperationEffect(new[] { Harness, Carrier }),
                ...
                new NNNActionDefinition(ShortTrip, "商店街ショートトリップ", "佐藤宅から危険な道路区間はキャリーで移動し、商店街でハーネスを使う短時間の外出を準備する。", NNNActionKind.Operation,
                    intentText: "キャリーとハーネスで商店街再訪が成立するか",
                    requiredKnowledgeTags: new[] { Walkable, BusyRoad, CatOnlyPaths, WantsOutside, HarnessUncertain }, discoveryKnowledgeTags: new[] { Walkable, BusyRoad },
